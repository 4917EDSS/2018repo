...
AutoRightToLeftScaleGrp::AutoRightToLeftScaleGrp() {
	...
	AddSequential(new SilkyMotionCmd(std::vector<double>{-800,2700}, std::vector<double>{35, 61}));

	AddParallel(new MoveElevatorToHeightCmd(ElevatorSub::SCALE_BOX_HIGH_HEIGHT));
	AddSequential(new SilkyMotionCmd(std::vector<double>{-2700, 800}, std::vector<double>{-61, -35}));
